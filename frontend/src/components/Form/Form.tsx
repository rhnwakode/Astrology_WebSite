...
import { locationApi } from '../../api/axiosConfig';
...
const Form = () => {
  ...
  // console.log(data);
  console.log(
    'debugging country and state status',
  ...
  const { setKundali } = kundaliContexts;

  const onSubmit: SubmitHandler<Inputs> = (datas) => {
    const sign = datas.date.getTimezoneOffset() >= 0 ? '-' : '+';
    const date: number = datas.date.getTimezoneOffset() * -1;
    let extraHr: string = Math.floor(date / 60).toString();
    extraHr = extraHr.length < 2 ? '0' + extraHr : extraHr;
    let extraMin: string = (date % 60).toString();

    extraMin = extraMin.length < 2 ? '0' + extraMin : extraMin;

    const url = encodeURI(
      `https://api.vedastro.org/Calculate/AllTimeData/Location/${datas.country.toLowerCase()}/${datas.state.toLowerCase()}/${datas.city.toLowerCase()}/Time/${
        datas.hrs
      }:${
        datas.min
      }/${datas.date.getDate()}/${datas.date.getMonth()}/${datas.date.getFullYear()}/${sign}${extraHr}:${extraMin}`,
    );
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await locationApi('/countries.json');
      const countryArray = res.data.map((obj) => ({
        id: obj.id,
        name: obj.name,
      }));
          ...
          <div className={style.field_time_hrs}>
            ...
            <input
              ...
              {...register('hrs', {
                ...
                max: {
                  value: 23,
                  message: 'Maximun can not be greater than 23',
                },
        ...
        <div className={style.field_location}>
          ...
          <div className={style.field_state}>
            ...
            {stateSuggestion.length > 0 && (
              // <select className={style.suggestion_wrapper}>
              //   {stateSuggestion.map((sug) => {
              //     return (
              //       <option key={sug} onClick={(e) => handleSelect(e, 'state')}>
              //         {sug}
              //       </option>
              //     );
              //   })}
